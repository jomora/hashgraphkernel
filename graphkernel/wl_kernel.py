...
def weisfeiler_lehman_subtree_kernel(graph_db, hashed_attributes, *kwargs):
    ...
    feature_vectors = []
    for _ in graph_db:
        feature_vectors.append(np.zeros(0, dtype=np.float32))
    ...
    if not compute_gram_matrix:
        return lil.lil_matrix(feature_vectors, dtype=np.float32)
    else:
        # Make feature vectors sparse
        gram_matrix = csr.csr_matrix(feature_vectors, dtype=np.float32)
        # Compute gram matrix
